...
import {
  createRetentionTime,
  deleteRetentionTime,
  getPageRetentionTimes,
  updateRetentionTime,
} from "./../../services/retention-time.service";
...
export default function OdorRetentionTime() {
  ...
  useEffect(() => {
    getPageRetentionTimes({ limit: LIMIT_PAGE_COMPONENT, page: currentPage })
      .then((result) => {
        setRetentionTimes(result.data[0]);
  ...
  const submit = useCallback(
    ({ value, id }) => {
      const exit = retentionTimes?.find(
    ...
    [reload, setReload, retentionTimes, title]
  );

  const submitRemove = (value) => {
    if (value) {
      deleteRetentionTime(idForRemove)
              ...
              <div className="modal-body">
                <div className="row">
                  <div className="col-md-12 col-sm-12 col-xs-12">
                    <FormGroup>
                      <Field
                        label="Thời gian lưu hương"
                        placeholder="Thời gian lưu hương"
                        required
                        id="value"
                        name="value"
            ...
            <Card className="shadow">
              ...
              <Table className="align-items-center" responsive>
                <thead className="thead-light">
                  <tr>
                    <th scope="col">STT</th>
                    <th scope="col">Thời gian lưu hương</th>
                    <th scope="col" />
                  </tr>
                </thead>
                <tbody>
                  {retentionTimes.map((_data, index) => (
                    <tr key={index}>
                      <th scope="row">
