...
export default function Style() {
  ...
  useEffect(() => {
    getPageStyles({ limit: LIMIT_PAGE_COMPONENT, page: currentPage })
      .then((result) => {
        setStyles(result.data[0]);
  ...
  const submit = useCallback(
    ...
    [reload, setReload, styles, title]
  );

  const submitRemove = (value) => {
    if (value) {
      deleteStyle(idForRemove)
